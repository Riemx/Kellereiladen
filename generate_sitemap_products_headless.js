...
// robots.txt beachten? (bei Bedarf false)
...
function looksLikeProduct(url){ return RE_PROD_END.test(url)||RE_PROD_ONLY.test(url)||RE_ITEM_PATH.test(url); }
...
function isInternal(url){ try{ const u=new URL(url); return u.hostname===HOST && /^https?:$/.test(u.protocol);}catch{return false;} }
...
function harvestIsbnsFromText(text){ const set=new Set(); for(const m of text.matchAll(RE_ISBN13)) set.add(m[0]); return Array.from(set); }
...
async function main(){
  ...
  const queue=[]; const seen=new Set(); const products=new Set();
  ...
  // Cache pflegen (Gnadenfrist)
  let cache={}; try{ cache=JSON.parse(fs.readFileSync(PRODUCTS_CACHE,"utf-8")); }catch{}
  const todayStr=today();
  for(const u of products){ cache[u]=cache[u]||{}; cache[u].last_seen=todayStr; cache[u].lastmod=cache[u].lastmod||todayStr; }
  const keep={}; for(const [u,meta] of Object.entries(cache)){ const ls=meta.last_seen||"1970-01-01"; const age=(new Date(todayStr)-new Date(ls))/(1000*60*60*24); if(age<=RETAIN_DAYS) keep[u]=meta; }
  cache=keep;
